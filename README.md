# [@devtea2026/doloribus-repellat-deleniti-nostrum][website]

[![Build Status](https://github.com/devtea2026/doloribus-repellat-deleniti-nostrum/workflows/ci/badge.svg)](https://github.com/devtea2026/doloribus-repellat-deleniti-nostrum/actions/workflows/ci.yml) [![Build Status](https://github.com/devtea2026/doloribus-repellat-deleniti-nostrum/workflows/rulesCoverage/badge.svg)](https://github.com/devtea2026/doloribus-repellat-deleniti-nostrum/actions/workflows/rulesCoverage.yml) [![npm package](https://img.shields.io/npm/v/@devtea2026/doloribus-repellat-deleniti-nostrum.svg)](https://www.npmjs.org/package/@devtea2026/doloribus-repellat-deleniti-nostrum) [![npm downloads](http://img.shields.io/npm/dm/@devtea2026/doloribus-repellat-deleniti-nostrum.svg)](https://www.npmjs.org/package/@devtea2026/doloribus-repellat-deleniti-nostrum)

English / [简体中文](./README.zh-CN.md)

**NEWS: @devtea2026/doloribus-repellat-deleniti-nostrum now support TypeScript 5.0**  
If you are using TypeScript 4.0, please `npm install --save-dev @devtea2026/doloribus-repellat-deleniti-nostrum@4`

**NEWS: @devtea2026/doloribus-repellat-deleniti-nostrum now support Vue 3.0**  
If you are using Vue 2.0, please `npm install --save-dev @devtea2026/doloribus-repellat-deleniti-nostrum@3`
...
Now that Prettier has become a necessary tool in front-end projects, @devtea2026/doloribus-repellat-deleniti-nostrum does not need to maintain the style-related rules in ESLint anymore, so we completely removed all Prettier related rules in the v3 version, and use ESLint to check logical errors which it's good at.
...
Therefore, @devtea2026/doloribus-repellat-deleniti-nostrum also inherits the philosophy of ESLint. It will not emphasize the need to use our config. Instead, we help you to make your config by reference our completed documents, examples, tests, websites, etc.

### High degree of automation: advanced rules management, test as a document, as a [website][]

> Relentless push automation

`@devtea2026/doloribus-repellat-deleniti-nostrum` uses a high degree of automation to hand over all processes that can be managed automatically, including:
...
And `@devtea2026/doloribus-repellat-deleniti-nostrum`, through the above-mentioned automated tools, can receive notifications from GitHub Actions at the first time, telling us which rules need to be added:

![](https://i.v2ex.co/zDK6Uao4.png)

In this way, we can follow the latest rules in a time when the front-end community is changing rapidly, and always keep the vitality and advanced of `@devtea2026/doloribus-repellat-deleniti-nostrum`.
...
```bash
npm install --save-dev eslint @babel/core @babel/eslint-parser @devtea2026/doloribus-repellat-deleniti-nostrum
```
...
```bash
npm install --save-dev eslint @babel/core @babel/eslint-parser @babel/preset-react@latest eslint-plugin-react @devtea2026/doloribus-repellat-deleniti-nostrum
```
...
```bash
npm install --save-dev eslint @babel/core @babel/eslint-parser vue-eslint-parser eslint-plugin-vue @devtea2026/doloribus-repellat-deleniti-nostrum
```
...
```bash
npm install --save-dev eslint typescript @typescript-eslint/parser @typescript-eslint/eslint-plugin @devtea2026/doloribus-repellat-deleniti-nostrum
```
...
```bash
npm install --save-dev eslint typescript @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-plugin-react @devtea2026/doloribus-repellat-deleniti-nostrum
```
...
```bash
npm install --save-dev @babel/core @babel/eslint-parser @typescript-eslint/eslint-plugin @typescript-eslint/parser @vue/eslint-config-typescript eslint @devtea2026/doloribus-repellat-deleniti-nostrum eslint-plugin-vue vue-eslint-parser
```
...
`@devtea2026/doloribus-repellat-deleniti-nostrum` does not include any style-related rules in v3, so there is no need to install `eslint-config-prettier`. Just install `prettier` and related VSCode plugins.

AlloyTeam provides a set of Prettier configuration, you can create a `.prettierrc` to reuse it:

```json
"@devtea2026/doloribus-repellat-deleniti-nostrum/.prettierrc.js"
```
...
Our team initially used Airbnb rules, but because it was too strict, some rules still needed to be personalized, which led to more and more changes in the future and finally decided to maintain a new set. After more than four years of maintaining, `@devtea2026/doloribus-repellat-deleniti-nostrum` is now very mature and progressive and has been welcomed by many teams inside and outside the company.
...
Therefore, `@devtea2026/doloribus-repellat-deleniti-nostrum` also inherits the philosophy of ESLint. It will not force you to use our config. Instead, we help you to _make your config_ by referencing our examples, tests, websites and so on.

### Why not `airbnb`

1. `@devtea2026/doloribus-repellat-deleniti-nostrum` has officially maintained `vue`, `typescript` and `react+typescript` rules. In contrast, airbnb's `vue` and `typescript` are maintained by third parties.
...
It's okay, `@devtea2026/doloribus-repellat-deleniti-nostrum` believes that different teams and projects can have different configurations from the design concept. Although you choose to use `airbnb`, you can still come to our [website][] when you have personalized configuration needs.
...
[website]: https://alloyteam.github.io/@devtea2026/doloribus-repellat-deleniti-nostrum/
